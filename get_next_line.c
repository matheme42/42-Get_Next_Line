...
/*   Updated: 2018/11/15 16:43:39 by matheme     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
t_glist			*find_fd(t_glist **lst, const int fd)
{
	while ((*lst)->next)
	{
		if ((*lst)->fd == fd)
...
int				get_next_line(const int fd, char **line)
{
	...
	if (!(lst))
	{
		if (!((lst) = malloc(sizeof(t_glist))))
			return (-1);
		(lst)->fd = fd;
		(lst)->line = ft_strnew(0);
		(lst)->next = NULL;
	}
	begin = lst;
	if (!(lst = find_fd(&lst, fd)))
		return (-1);
	if ((ret = ft_getline(fd, &lst->line)) == -1)
		return (-1);
